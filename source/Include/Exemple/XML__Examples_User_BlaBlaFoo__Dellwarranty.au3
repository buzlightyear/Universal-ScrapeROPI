...
Func _Tasks_XML()
	...
	Local $sXmlFile = @ScriptDir & "\XML_Example_XML_Files\Dellwarranty_Tidy.xml"
	...
	Local $aDellWarranty[4]
	MsgBox(0, '', $oXMLDoc.XML)
;~ 	$oXMLDoc.setProperty("SelectionNamespaces",    'xmlns="http://schemas.datacontract.org/2004/07/Dell.Support.AssetsExternalAPI.Web.Models.V1.Response"');
;~ 	$oXMLDoc.setProperty("SelectionLanguage", "XPath");

;~ 	Local $oNodes_List = $oXmlDoc.selectNodes('AssetWarrantyResponse/*')
;~ 	Local $oNodes_List = $oXmlDoc.selectNodes('*/AssetWarrantyResponse/AssetEntitlementData[0]/*')
	Local $oNodes_List = $oXmlDoc.selectNodes('*/*/*/*/*')
;~ 	Local $oNodes_List = $oXmlDoc.selectNodes('//*')
	If @error Then MsgBox($MB_ICONERROR, '', '@error = ' & @error & @CRLF & '@extended = ' & @extended)
	For $oNode_enum In $oNodes_List
;~ 		ConsoleWrite($oNode_enum.text & @CRLF)
		ConsoleWrite($oNode_enum.nodename  & ' = ' & $oNode_enum.text   & @CRLF)
	Next
	$aDellWarranty[0] = $oXmlDoc.selectSingleNode('//EndDate')
;~ 	$aDellWarranty[0] = $oXmlDoc.selectNodes('//EndDate[0]')
	If @error Then MsgBox($MB_ICONERROR, '', '@error = ' & @error & @CRLF & '@extended = ' & @extended)

;~ 	$aDellWarranty[0] = $oXmlDoc.selectSingleNode('//AssetWarrantyResponse').text
	$aDellWarranty[1] = $oXmlDoc.selectSingleNode('//EntitlementType')
	$aDellWarranty[2] = $oXmlDoc.selectSingleNode('//MachineDescription')
	$aDellWarranty[3] = $oXmlDoc.selectSingleNode('//ServiceTag')

	_ArrayDisplay($aDellWarranty, '$aDellWarranty')
